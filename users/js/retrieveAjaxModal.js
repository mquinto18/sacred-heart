...
$("#updatetestchange").val(data.remarks_event);
...
$(document).ready(function(){
	$("#baptist_entry_reserve_date").datepicker({
		dateFormat: "yy-mm-dd",
		changeMonth: true,
		changeYear: true,
		yearRange: "1900:c"
...
// entry baptist
$(document).ready(function(){
	$("#entryBaptist").change(function(){
		let entryBaptist = $("#entryBaptist").val();
		if (entryBaptist == "Online Payment") {
			$("#entryBaptistFile").show();
			$(".file").attr("required", "required");
		}else{
			$("#entryBaptistFile").hide();
			$(".file").removeAttr("required");
		}
	});
});
